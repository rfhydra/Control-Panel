<?php
include_once('./includes/main/validation/exclude/validation_recover_fireguard.php');
include_once('./includes/main/function/external/func_user_recover_fireguard.php');
...
$leftTitle = _l('Recover Fireguard');
...
if (count($_GET) > 0) {
    ...
    if (isset($config['security_recaptcha_enable']) && $config['security_recaptcha_enable'] == 1 && empty($_POST['g-recaptcha-response'])) {
        ...
        valdiateRecoverFgField($username, $password, $email, $pin);
        executeRecoverFireguard($username, $password, $email, $pin);
        if (!empty($message)) {
            displayError($message);
        } else {
            $out .= '<div class="alert alert-success">';
            $out .= '   <h4>Recover Fireguard Success!</h4>';
            $out .= '   <p>Username : '. trim($rowUser['id']) .'</p>';
            $out .= '   <p>Fireguard : '. trim($rowUser['fg']) .'</p>';
            $out .= '   <p>Hint : '. trim($rowUser['hint']) .'</p>';
            $out .= '</div>';
        }
    }
}
...
$out .= '<h3 class="panel-title">RECOVER YOUR FIREGUARD HERE</h3>';
$out .= '</div>';
...
$out .= '<td><input type="password" autocomplete="off" class="form-control" name="password" placeholder="Password" maxlength="12" value="'. $password .'" pattern="[a-zA-Z0-9]{4,12}" required=""></td>';
$out .= '</tr>';
$out .= '<tr>';
...
$out .= '<button type="submit" class="btn btn-success">Recover Fireguard</button><input type="hidden" name="page" value="recover"></td>';
$out .= '</tr>';
$out .= '</tbody></table>';
$out .= '</div>';
$out .= '</div>';
